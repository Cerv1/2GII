...
int finder(int begin, int end, int umbral, const vector<int> & myvector){
    int medio=(begin+end)/2;

    int res=-1; if((end-begin)>umbral){

	if(medio == myvector[medio])
	    res = medio;
    else
	    res = finder(begin, medio-1, umbral, myvector);

	if (res == -1)
	    res = finder(medio+1, end, umbral, myvector);
    }
    else{
        bool rompe=false;
...
int main(int argc, char * argv[])
{
  ...
  int umbral = (atoi(argv[1]))/0.15;     // nuestro umbral sera el 15% del num de elementos
  if (argc != 2)
    {
