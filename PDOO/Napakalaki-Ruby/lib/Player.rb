...
class Player
    ...
    #EXAMEN
    def stealToPreviousPlayer()
      napa=Napakalaki.instance
      anterior=napa.getCurrentPlayer
      anterior=anterior-1
      jugadores=Array.new
      jugadores=napa.players
      if(anterior==-1)
        anterior=jugadores.size()-1
      end
      previo=Player.new(jugadores[anterior])
      gotBetter=false
      mejorBonus=0
      posBonus=-1
      for i in 0..(previo.hiddenTreasures.size)
        if(previo.instance_of?(CultistPlayer))
          if(previo.hiddenTreasures[i].bonus > mejorBonus&& previo.hiddenTreasures[i].type != TreasureKind::ONEHAND)
            mejorBonus=previo.hiddenTreasures[i].bonus
            posBonus=i
            gotBetter=true
          end
        else
           if(previo.hiddenTreasures[i].bonus > mejorBonus)
             mejorBonus=previo.hiddenTreasures[i].bonus
              posBonus=i
              gotBetter=true
           end
        ...
        if(posBonus!=-1)
          mejorTesoro=previo.hiddenTreasures[posBonus]
        end
        
        if(canISteal() && gotBetter==true)
          if(canMakeTreasureVisible(mejorTesoro))
            @visibleTreasures<<mejorTesoro
          else
            @hiddenTreasures<<mejorTesoro
            discardHiddenTreasure(mejorTesoro)
          end
        end
    end
    #FIN EXAMEN
